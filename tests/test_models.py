...
@pytest.mark.parametrize('kernel_sizes', [
    [],
    [(1, 2, 3)],
    [(1, 2, 2), (3, 1, 1)],
])
...
@pytest.mark.parametrize('flatten_style', ['keep_chans', 'keep_both'])
def test_model_forward_backward_works(
        mellin, mconv_decimation_strategy, flatten_style,
        window, freq_dim, target_dim, kernel_sizes, hidden_sizes):
    params = models.AcousticModelParams(
        seed=1, mellin=mellin,  target_dim=target_dim, freq_dim=freq_dim,
        kernel_sizes=kernel_sizes, hidden_sizes=hidden_sizes,
        mconv_decimation_strategy=mconv_decimation_strategy,
...
def test_model_to_gpu(mellin):
    params = models.AcousticModelParams(
        seed=2, mellin=mellin, target_dim=10,
        freq_dim=10,
    )
