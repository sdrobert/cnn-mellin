...
#     except ImportError:  # optuna missing
#         pass
#     return dict_
...
#     def serialize(self, name, parameterized):
#         val = super(CommaTupleListSerializer, self).serialize(name, parameterized)
#         return ", ".join("(" + ", ".join(str(xx) for xx in x) + ")" for x in val)
...
#                             raise ValueError("closing parenthesis without open")
#                         x = x[:-1].strip()
#                         append = True
...
#     def help_string(self, name, parameterized):
#         choices_help_string = super(CommaListSelectorSerializer, self).help_string(
#             name, parameterized
#         )
#         return "Elements separated by commas. " + choices_help_string

#     def serialize(self, name, parameterized):
#         val = super(CommaListSelectorSerializer, self).serialize(name, parameterized)
#         return ", ".join(str(x) for x in val)
...
#     parser = argparse.ArgumentParser(description=print_parameters_as_ini.__doc__)
#     parser.add_argument(
#         "in_configs",
#         nargs="*",
...
#             study_name=param_dict["optim"].study_name, storage=options.history_url
#         )
#         data_set_params = data.ContextWindowDataSetParams(
#             name="data_set",
#             **param.param_union(
#                 param_dict["data"], param_dict[options.data_set_config_section],
#             )
#         )
...
#             (param_dict[options.data_set_config_section], data_set_params),
#         ):
...
#     parser = argparse.ArgumentParser(description=target_count_info_to_tensor.__doc__)
#     parser.add_argument(
#         "--num-targets",
#         type=int,
#         default=None,
...
#         type=int,
#         default=1,
#         help="If a count is less than this value, it will be increased to this" "value",
#     )
#     parser.add_argument("in_file", type=argparse.FileType("r"), help="The info file")
#     parser.add_argument(
#         "tensor_type",
#         choices=["inv_weight", "log_prior"],
...
#     parser = argparse.ArgumentParser(description=acoustic_model_forward_pdfs.__doc__)
#     param_dict = _construct_default_param_dict()
#     parser.add_argument(
#         "--config",
...
#         "--pdfs-dir",
#         default=None,
#         help="Directory to save pdfs to. If unset, will write to data_dir + " "'/pdfs'",
#     )
#     parser.add_argument(
#         "log_prior_file",
...
#     )
#     history_parser.add_argument(
#         "state_csv", help="A path to the CSV file where the training history is listed"
#     )
#     history_parser.add_argument(
#         "--last",
#         default=False,
...
#         1 + options.config["data"].context_left + options.config["data"].context_right,
#     )
...
#                 options.state_dir, controller.params.saved_model_fmt.format(**info)
#             )
#         else:
#             warnings.warn(
#                 'There are no training logs listed in "{}"! pdfs will be ' "random"
#             )
#             model_path = None
#         del controller
...
#             name="pdfs_data_set",
#             **param.param_union(options.config["data"], options.config["pdfs_data"],)
#         ),
#         pin_memory=(options.device.type == "cuda"),
#     )
#     running.write_am_pdfs(
#         model, pdfs_data, log_prior, device=options.device, pdfs_dir=options.pdfs_dir,
#     )
#     return 0


# def _train_acoustic_model_parse_args(args):
#     parser = argparse.ArgumentParser(description=train_acoustic_model.__doc__)
#     param_dict = _construct_default_param_dict()
#     parser.add_argument(
#         "--config",
#         action=pargparse.ParameterizedIniReadAction,
...
#     )
#     parser.add_argument(
#         "state_dir", help="Path to where model and optimizer states are stored"
#     )
...
#             options.weight_tensor_file, map_location=options.device,
#         )
#     else:
#         weight_tensor = None
#     train_params = data.ContextWindowDataSetParams(
#         name="train_data_set",
#         **param.param_union(options.config["data"], options.config["train_data"],)
#     )
...
#     parser = argparse.ArgumentParser(description=optimize_acoustic_model.__doc__)
#     param_dict = _construct_default_param_dict()
#     parser.add_argument(
#         "--config",
#         action=pargparse.ParameterizedIniReadAction,
...
#     )
#     parser.add_argument(
#         "out_ini", help="Path to write the optimize config (INI) file to"
#     )
#     return parser.parse_args(args)
...
#         else:
#             partitions = tuple(
#                 tuple(x.strip() for x in open(f)) for f in options.partitions
#             )
#     except ValueError:
#         print(
#             "partitions must be an integer or sequence of file names", file=sys.stderr
#         )
#         return 1
...
#         name="data_set",
#         **param.param_union(
#             options.config["data"], options.config[options.data_set_config_section],
#         )
#     )
#     model_params, training_params, data_set_params = optim.optimize_am(
#         options.data_dir,
#         partitions,
#         options.config["optim"],
#         options.config["model"],
#         options.config["training"],
#         data_set_params,
