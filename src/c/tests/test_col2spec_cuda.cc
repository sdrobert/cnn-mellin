...
int main(int argc, const char* argv[]) {
  ...
  CheckCuda(cudaMallocManaged(&g, sizeof(double) * batch * c_in * ngx * ngy));  // unif
  CheckCuda(cudaMalloc(&h, sizeof(double) * batch * c_in * nfx * nfy * (nhx + extra_h) * nhy)); // device
  ...
  CopyHCuda(H, batch, c_in, nfx * nfy, nhx * nhy, 0, nhy * extra_h, transposed_h, h);
