...
int main(int argc, const char* argv[]) {
  ...
  if (test_idx < mconv1db::kNumTests) {
    ...
    ux = mconvlconvb::kUX[test_idx]; uy = mconvlconvb::kUY[test_idx];
  ...
  CopyFCpu(F, c_in, c_out, nfx * nfy, transposed_f, f);
  CopyGOrHCpu(G, batch, c_in, ngx, ngy, 0, 0, transposed_g, g);
  CopyGOrHCpu(H, batch, c_out, nhx, nhy, extra_h, extra_h, transposed_h, h_exp);
  double h[10000] = {0.};

  cdrobert::mellin::MConvLConv(f, g,
                               c_out, c_in, batch,
                               nfx, nfy, ngx, ngy, nhx + extra_h, nhy + extra_h,
