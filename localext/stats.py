...
def main(args=None):
    ...
    parser.add_argument(
        ...
        type=compile,
        default=compile(
            r'^%WER (?P<wer>[\d\.]+) .*/(?P<feature>[^_]+)_(?P<model>[^/]+)/'
            r'(?P<trial>\d+)/decode_(?P<partition>[^/]+)/.*')
    )
    parser.add_argument(
        '--continuous',
        default='wer',
    ...
    parser_descriptive.add_argument(
        ...
        default=['partition', 'feature', 'model'],
        help='A list of independent variables. Stats will be calculated for '
    ...
    parser_wilcoxon.add_argument(
        '--independent-variable', default='feature',
